	  				...
	  				<div class="pd-1-25 pd-r-15 pd-b-25">
						  ...
						  <tbody>
						    ...
						    <tr>
					      	...
					      	@foreach( App\Models\Backend\category::orderBy('category_name','asc')->where('id',$product->category_id)->get() as $category )
					      	...
					      	@foreach( App\Models\Backend\brand::orderBy('name','asc')->where('id',$product->brand)->get() as $band )
					      		{{ $band->name }}
					      	@endforeach
					      </td>
						      ...
						      <th scope="row">{{ $product->short_desc }}</th>
						      <th scope="row">{{ $product->desc }}</th>
